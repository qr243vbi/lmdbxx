...
#include <string_view> /* for LMDBXX_STRING_VIEW */
#else
 ...
 */
#ifdef MDB_SIZE_MAX
  #define LMDBXX_SIZE_T mdb_size_t
#elif defined(MDB_VL32)
#error "Although MDB_VL32 is defined, it is not supported by the underlying LMDB version."
#else
  #define LMDBXX_SIZE_T std::size_t
#endif
#ifdef LMDBXX_USE_EXPERIMENTAL_STRING_VIEW
  #define LMDBXX_STRING_VIEW std::experimental::string_view
#else
...
namespace lmdb {
  ...
  static inline void env_set_mapsize(MDB_env* env, LMDBXX_SIZE_T size);
...
#ifdef LMDBXX_TXN_ID
  static inline LMDBXX_SIZE_T txn_id(MDB_txn* txn) noexcept;
#endif
...
namespace lmdb {
  ...
  static inline void cursor_count(MDB_cursor* cursor, LMDBXX_SIZE_T& count);
}
...
public:
   ...
   */
  LMDBXX_STRING_VIEW get_internal_map() {
    ...
    return LMDBXX_STRING_VIEW(me_map, arg.me_mapsize);
  }
};
...
public:
   ...
   * @throws lmdb::error on failure
   */
  LMDBXX_SIZE_T size(MDB_txn* const txn) const {
  ...
  bool get(MDB_txn* const txn,
           const LMDBXX_STRING_VIEW key,
           LMDBXX_STRING_VIEW& data) {
    ...
    if (ret) {
        data = LMDBXX_STRING_VIEW(static_cast<char*>(dataV.mv_data), dataV.mv_size);
    }
    return ret;
  }
...
public:
   ...
   * @throws lmdb::error on failure
   */
  bool get(LMDBXX_STRING_VIEW &key,
           const MDB_cursor_op op) {
    MDB_val keyV{key.size(), const_cast<char*>(key.data())};
    bool ret = lmdb::cursor_get(handle(), &keyV, nullptr, op);
    if (ret) {
        key = LMDBXX_STRING_VIEW(static_cast<char*>(keyV.mv_data), keyV.mv_size);
    }
    return ret;
  }
  ...
  bool get(LMDBXX_STRING_VIEW &key,
    ...
    if (ret) {
        key = LMDBXX_STRING_VIEW(static_cast<char*>(keyV.mv_data), keyV.mv_size);
        val = LMDBXX_STRING_VIEW(static_cast<char*>(valV.mv_data), valV.mv_size);
    }
    return ret;
  }
   ...
   * @throws lmdb::error on failure
   */
  bool put(const LMDBXX_STRING_VIEW &key,
           const LMDBXX_STRING_VIEW &val,
           const unsigned int flags = 0) {
    MDB_val keyV{key.size(), const_cast<char*>(key.data())};
    MDB_val valV{val.size(), const_cast<char*>(val.data())};
...
namespace lmdb {
  /**
   * Creates a LMDBXX_STRING_VIEW that points to the memory pointed to by v.
   *
   * @param v
   */
  template<typename T>
  static inline LMDBXX_STRING_VIEW ptr_to_sv(T* v) {
    return LMDBXX_STRING_VIEW(reinterpret_cast<char*>(v), sizeof(*v));
  }

  /**
   * Creates a LMDBXX_STRING_VIEW that points to the memory occupied by v.
   *
   * @param v
   */
  template<typename T>
  static inline LMDBXX_STRING_VIEW to_sv(const T &v) {
    return LMDBXX_STRING_VIEW(reinterpret_cast<const char*>(std::addressof(v)), sizeof(v));
  }

  /**
   * Takes a LMDBXX_STRING_VIEW and casts its pointer as a pointer to the parameterized type.
   *
   * @param v
   */
  template<typename T>
  static inline T* ptr_from_sv(LMDBXX_STRING_VIEW v) {
    if (v.size() != sizeof(T)) error::raise("ptr_from_sv", MDB_BAD_VALSIZE);
    return reinterpret_cast<T*>(const_cast<char*>(v.data()));
  }

  /**
   * Takes a LMDBXX_STRING_VIEW and dereferences it, returning a value of the parameterized type.
   *
   * @param v
   */
  template<typename T>
  static inline T from_sv(LMDBXX_STRING_VIEW v) {
